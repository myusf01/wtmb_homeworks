...
test("Undo a retweet.", async t => {
    ...
    const updatedTweet1v1 = (await request(app)
        .get(`/tweet/${tweet1.body._id}/json`)).body
    const updatedUser2v1 = (await request(app)
        .get(`/user/${user2.body._id}/json`)).body

    
    console.log(retweet1.body._id);
    const undoRetweet = await request(app)
        .post(`/retweet/undo/${retweet1.body._id}`)
    
    t.is(undoRetweet.status,200)

    const updatedTweet1v2 = (await request(app)
        .get(`/tweet/${tweet1.body._id}/json`)).body
    const updatedUser2v2 = (await request(app)
        .get(`/user/${user2.body._id}/json`)).body
...
test("Retweet a retweeted tweet", async t => {
    ...
    const updatedUser2v2 = (await request(app)
        .get(`/user/${user2.body._id}/json`)).body
    const updatedUser1v2 = (await request(app)
        .get(`/user/${user1.body._id}/json`)).body
